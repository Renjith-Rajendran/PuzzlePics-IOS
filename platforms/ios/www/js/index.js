...
function onDeviceReady() {
   ...
   /* if(states[networkState] != "No network connection" && states[networkState] != "Unknown connection")
    ...
    if ((changeImageFlagStatus === undefined) ||(changeImageFlagStatus == null)){
        console.log("ChangeImageFlagStatus :"+changeImageFlagStatus)
    }
    ...
    if ((settingPageChangeStatus === undefined) ||(settingPageChangeStatus == null)){
        console.log("SettingPageChangeStatus : "+settingPageChangeStatus)
    }
...
$('#settingsShape').click( function(e){
                          ...
                          window.localStorage.setItem("Puzzle_ShapeId",shapeId);
                          window.localStorage.setItem("Puzzle_SettingPageChange","true");
                          console.log("ShapeStatus : "+shapeStatus+"ShapeId : "+shapeId);
                          });
...
function getImages_play(tx, results) {
    ...
    for (var i=0; i<len; i++) {
        ...
        window.localStorage.setItem("Puzzle_PlayImage", imageListItems.image_path);
        ...
        playImage =imageListItems.image_path;
        console.log("PlayImage : "+playImage);
    }
    playPuzzleGame();
}
...
function playPuzzleGame()
{
    ...
    console.log("shapeId  : "+shapeIdVal);
    console.log("playImage  : "+playImage);
    console.log("levelPieceVal  : "+levelPieceVal);
    switch (shapeIdVal) {
        case "1":
...
function downloadImageRecived(URL,id,image_name)
{
    DownloadFile(URL, id, image_name);
}

function DownloadFile(URL, id, image_name) {
    ...
    else
    {
        ...
        {
           download(URL, id, image_name); //If available download function call
        }
    }
}

function download(URL, id, image_name) {
    window.requestFileSystem(LocalFileSystem.PERSISTENT, 1, fileSystemSuccessImage, fileSystemFail);
    function fileSystemSuccessImage(fileSystem)
    {
        var download_link = encodeURI(URL);
        ext = download_link.substr(download_link.lastIndexOf('.') + 1); //Get extension of URL
        var path = cordova.file.dataDirectory  + image_name + "." + ext;
        
        filetransferRecivied(download_link, path,id);
    }
    ...
    function filetransferRecivied(download_link, fp,id) {
        ...
        fileTransfer.download(download_link, fp,
                              ...
                              window.localStorage.setItem("Puzzle_camera_gallery", "true");
                              var status = "Received";
                              var purchase_status = "y";
                              db.transaction( function(tx)
                                             {	tx.executeSql("INSERT INTO imageListParsed (row_id, image_path, status, purchase_status) VALUES ('" + id + "','" +  smallImage.src + "','" + status  + "','" + purchase_status + "');", [],   insertRecivied, errorCB);
                                             }, transaction_error);
...
function ImagePurchaseStatus(strResult)
{
    ...
    $.ajax(
           ...
           dataType: "json",
           success: function(msg) {
            updateImageTable(msg,strResult);
           },
           error: function() {
           alert('error');
           }
...
function updateImageTable(msg,id)
{
	var purchase_status = "Y"
    db.transaction( function(tx)
                   {	tx.executeSql("INSERT INTO imageListParsed (row_id, purchase_status) VALUES ('" + id + "','" + purchase_status + "');", [], updatePurchase, errorCB);
                   }, transaction_error);
}
...
$('#downloadImage').click( function(e){
    $('.loginPage, .registerPage, .settingsPage,.popUpCov,.imgSelectPage,.recivied').hide();
    $('.progressBar').show();
    var tokenValue = document.getElementById("token").value;
    Url = "http://www.polussoftware.com/puzzlepics/popup_screen.php?Token=" + token_Value;
    $.ajax(
    {
        type: "POST",
        url: Url,
    ...
    function recivedImage(result)
    {
        ...
        $.each(result.data, function(key,value)
        {
            if( value.id == "invalid token"){
               ...
               console.log("url " +value.image_url );
               var url = "http://www.polussoftware.com/puzzlepics/"+value.image_url;
               downloadImageRecived(url, value.id , value.image_name);            }
        })
    }
});
/* End Recived token */
...
function downloadImage(URL, Folder_Name, File_Name,server_row_id,image_status,purchase_status) {
    window.requestFileSystem(LocalFileSystem.PERSISTENT,1, fileSystemSuccess, fileSystemFail);
...
function filetransfer(download_link, fp,server_row_id,image_status,purchase_status) {
    ...
    fileTransfer.download(download_link, fp,
                          ...
                          console.log("filetransfer Row Id " +server_row_id  );
                          ...
                          console.log("Completed File: " + entry.fullPath);
                          },
                          function(error) {
                          console.log("download error source " + error.source);
                          console.log("download error target " + error.target);
                          ...
                          {
                          headers: {
                          "Authorization": "Basic dGVzdHVzZXJuYW1lOnRlc3RZA=="
                          }
                          }
                          );
}
...
function getImagesServer_success(tx, results) {
    ...
    for (var i=0; i<len; i++) {
        var imageListItems = results.rows.item(i);
        console.log("GetImagesServer_success : "+imageListItems.image_path )
        if(imageListItems.status == "Free")
        {
           // $('.galleryAvilable').show();
        ...
        else if(imageListItems.status == "Received" )
        {
           // $('.galleryReceived').show();
            var newlist='<li><a href="#" onClick="ImageClick(' + imageListItems.row_id + ')"><img src="' + imageListItems.image_path + '"/> </a></li>';
