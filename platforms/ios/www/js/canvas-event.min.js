...
/* var canvas = document.getElementById('canvas');
 ...
 //alert("moving");
 //alert("width canvas " +  canvas.width + "\n height canvas "+ canvas.height );
 // alert("width  self" +  self.x + "\n height self "+ self.y );
 if (this === curDrag) {
 if (!inDrag) {
 ...
 // alert("width  c.x" + c.x + "\n height c.y "+ c.y );
 // alert("width  c.lastX " + c.lastX + "\n height c.lastY "+ c.lastY );
 if((c.x > 50) && (c.x < ( canvas.width - 50)) && (c.y > 50) && (c.y < (canvas.height -130) ))
 {
 // var x = c.x - c.lastX;
 // var y = c.y - c.lastY;
 ...
 window.localStorage.setItem("self.x",c.x);
 window.localStorage.setItem("self.y",c.y);
 end.call(this, c, e)
 }
 }
 }
